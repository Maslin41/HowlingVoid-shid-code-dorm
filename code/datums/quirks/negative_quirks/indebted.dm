/datum/quirk/indebted
	name = "Indebted(В долгу)"
	desc = "Неудачные решения в жизни, медицинские счета, студенческие кредиты — что бы это ни было, вы накопили солидный долг. Часть всех полученных вами денег пойдёт на его погашение."
	icon = FA_ICON_DOLLAR
	quirk_flags = QUIRK_HUMAN_ONLY|QUIRK_HIDE_FROM_SCAN
	value = -2
	medical_record_text = "Увы, пациенту с трудом удалось собрать достаточно денег, чтобы оплатить счет за обследование. На диагнз этого не хватило."
	hardcore_value = 2
...
/datum/quirk/indebted/add_unique(client/client_source)
	...
	to_chat(client_source.mob, span_warning("Блять! У меня задолжности по кредиту в: [debt]..."))
