...
/datum/quirk/all_nighter
	name = "All Nighter(Бессонник)"
	desc = "Тебе не спалось прошлой ночью, и это видно! Ты будешь постоянно в плохом настроении и обладаешь склонностью спать дольше. Однако стимуляторы или короткий сон могут помочь."
	icon = FA_ICON_MOON
	value = -4
	mob_trait = TRAIT_HEAVY_SLEEPER
	gain_text = span_danger("Ты чувствуешь себя усталым и измотанным.")
	lose_text = span_notice("Ты наконец-то выспался и чувствуешь себя лучше.")
	medical_record_text = "Пациент, по-видимому, страдает от недостатка сна."
	hardcore_value = 2
...
/datum/quirk/all_nighter/process(seconds_per_tick)
	...
	if(!all_nighter && !happy_camper)
		quirk_holder.add_mood_event("all_nighter", /datum/mood_event/all_nighter)
		to_chat(quirk_holder, span_danger("Ты снова начинаешь чувствовать усталость.."))
