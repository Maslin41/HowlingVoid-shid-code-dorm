/datum/quirk/prosthetic_organ
	name = "Prosthetic Organ(Протез органа)"
	desc = "Из-за несчастного случая ты потерял один из органов. Поэтому теперь у тебя лишний протез!"
	icon = FA_ICON_LUNGS
	value = -3
	medical_record_text = "При физикальном обследовании у пациента был обнаружен малобюджетный протез органа. \
...
/datum/quirk/prosthetic_organ/add_unique(client/client_source)
	...
	switch(organ_slot)
		...
		if(ORGAN_SLOT_STOMACH)
			...
			slot_string = "stomach"
	medical_record_text = "Во время медицинского осмотра у пациента был обнаружен малобюджетный протез. [slot_string]. \
...
/datum/quirk/prosthetic_organ/post_add()
	to_chat(quirk_holder, span_bolddanger("Ваш [slot_string] был заменён лишним органом. Он слаб и крайне нестабилен. \
	Кроме того, любой ЭМИ полностью прекратит работу.."))
