...
/mob/living/proc/can_enter_vent(obj/machinery/atmospherics/components/ventcrawl_target, provide_feedback = TRUE)
	...
	if(buckled)
		...
		return
	if(iscarbon(src) && required_nudity && !signal_result) // Howling Void Edit
...
/mob/living/proc/handle_ventcrawl(obj/machinery/atmospherics/components/ventcrawl_target)
	...
	if(HAS_TRAIT(src, TRAIT_MOVE_VENTCRAWLING) && istype(loc, /obj/machinery/atmospherics) && movement_type & VENTCRAWLING)
		...
		if(has_client && isnull(client))
			return
		if(!do_after(src, exit_time, target = ventcrawl_target)) // Howling Void Edit
			SEND_SIGNAL(src, COMSIG_VENTCRAWL_PRE_CANCEL, ventcrawl_target) // Howling Void Addition
			return
		...
		update_pipe_vision()
		SEND_SIGNAL(src, COMSIG_VENTCRAWL_EXIT, ventcrawl_target) // Howling Void Addition
	...
	//Entrance here
		...
		if(vent_parent && (vent_parent.members.len || vent_parent.other_atmos_machines))
			// Howling Void Addition
			var/enter_time = SEND_SIGNAL(src, COMSIG_VENTCRAWL_PRE_ENTER, ventcrawl_target)
			if(!enter_time)
			...
			if(!do_after(src, enter_time, target = ventcrawl_target, extra_checks = CALLBACK(src, PROC_REF(can_enter_vent), ventcrawl_target))) // Howling Void edit
				SEND_SIGNAL(src, COMSIG_VENTCRAWL_PRE_CANCEL, ventcrawl_target) // Howling Void addition
				return
			if(has_client && isnull(client))
				return
			...
			SEND_SIGNAL(src, COMSIG_VENTCRAWL_ENTER, ventcrawl_target) // Howling Void Addition
		else
