/// Generates flavor text for the heretic's roundend report based on their path, success, and fate
/datum/antagonist/heretic/proc/get_flavor(cultiewin, ascended, heretic_path)
	...
	if(heretic_path == PATH_ASH)
		...
		if(ascended)
			...
			if(escaped)
				flavor_message += 	"What. A. PERFORMANCE! You may not have had your encore, but the emotion, oh the EFFORT! The night waits for you to seize the day once more, until then, the moon will howl to you - telling stories of what joys you could bring..."
				message_color = "#008000"
			else if(alive)
