...
/datum/action/cooldown/spell/aoe/moon_ringleader/cast_on_thing_in_aoe(mob/living/carbon/victim, mob/living/caster)
	...
	var/damage_change = victim.adjust_organ_loss(ORGAN_SLOT_BRAIN, 100 - victim_sanity, 160)
	...
	var/datum/status_effect/heretic_passive/moon/moon_passive = caster.has_status_effect(/datum/status_effect/heretic_passive/moon)
	if(moon_passive?.passive_level >= 3 && damage_change < 0)
		var/obj/item/organ/brain/caster_brain = caster.get_organ_slot(ORGAN_SLOT_BRAIN)
		if(caster_brain)
			caster_brain.apply_organ_damage(damage_change * 0.5)
