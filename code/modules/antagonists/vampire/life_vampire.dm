...
/datum/antagonist/vampire/proc/handle_healing()
	...
	var/burn_heal = min(carbon_owner.get_fire_loss(), actual_regen) * (HAS_TRAIT(carbon_owner, TRAIT_NOFIRE) ? 0.3 : 0.5) // if they have nofire, they get less innate burn healing as a compromise
	...
	if(in_torpor)
		...
		if(istype(carbon_owner.loc, /obj/structure/closet/crate/coffin))
			...
			burn_heal = min(carbon_owner.get_fire_loss(), actual_regen)
			...
			INVOKE_ASYNC(carbon_owner, TYPE_PROC_REF(/mob/living/carbon, remove_all_embedded_objects))

			if(try_regenerate_limbs(vitaecost_multiplier))
				return TRUE
		else
			burn_heal = min(carbon_owner.get_fire_loss(), actual_regen) * 0.8
...
/datum/antagonist/vampire/proc/try_regenerate_limbs(cost_muliplier = 1)
	...
	for(var/missing_limb in missing) //Find ONE Limb and regenerate it.
		...
		playsound(carbon_owner, 'sound/effects/magic/demon_consume.ogg', 50, TRUE)
		return TRUE
...
/datum/antagonist/vampire/proc/handle_blood()
	...
	if(current_vitae < VAMPIRE_LOW_BLOOD_WARNING && !low_blood_alerted)
		owner.current.playsound_local(null, 'code/modules/antagonists/vampire/sound/bloodneed.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/vampire/proc/final_death(skip_destruction = FALSE)
	...
	if(!skip_destruction)
		if(iscarbon(body))
			...
			INVOKE_ASYNC(carbon_body, TYPE_PROC_REF(/mob/living/carbon, remove_all_embedded_objects))
			playsound(owner.current, 'code/modules/antagonists/vampire/sound/burning_death.ogg', 100, TRUE)
		else
