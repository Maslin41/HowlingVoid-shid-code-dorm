...
/datum/action/cooldown/vampire/targeted/summon
	...
	ranged_mousepointer = 'code/modules/antagonists/vampire/icons/vampire_summon.dmi'
...
/datum/action/cooldown/vampire/targeted/summon/fire_targeted_power(atom/target_atom, list/modifiers)
	...
	carbon_target.playsound_local(null, 'code/modules/antagonists/vampire/sound/mesmerize.ogg', 70, FALSE, pressure_affected = FALSE)
...
/datum/status_effect/summoned/proc/start_movement()
	...
	if(QDELETED(source_vampire) || QDELETED(owner))
		return
	move_loop = GLOB.move_manager.home_onto(owner, source_vampire, step_delay, timeout = INFINITY)
	if(move_loop)
