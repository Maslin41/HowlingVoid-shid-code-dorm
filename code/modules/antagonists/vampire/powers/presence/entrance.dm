...
/datum/action/cooldown/vampire/targeted/entrance
	...
	ranged_mousepointer = 'code/modules/antagonists/vampire/icons/vampire_entrance.dmi'
...
/datum/action/cooldown/vampire/targeted/entrance/check_valid_target(atom/target_atom)
	...
	if(IS_CURATOR(carbon_target))
		...
		return FALSE
	if(carbon_target.can_block_magic(MAGIC_RESISTANCE_MIND, charge_cost = 1))
		return FALSE
...
/datum/action/cooldown/vampire/targeted/entrance/fire_targeted_power(atom/target_atom, list/modifiers)
	...
	to_chat(owner, span_notice("You capture [carbon_target]'s attention, leaving [carbon_target.p_them()] dazed."), type = MESSAGE_TYPE_INFO)

	carbon_target.playsound_local(null, 'code/modules/antagonists/vampire/sound/mesmerize.ogg', 50, FALSE, pressure_affected = FALSE)
