...
/datum/action/cooldown/vampire/astral_projection/activate_power()
	. = ..()
	owner.playsound_local(null, 'code/modules/antagonists/vampire/sound/auspex.ogg', 50, TRUE, pressure_affected = FALSE)
	...
	ghost.add_filter("astral_projection", 1, outline_filter(size = 1, color = LIGHT_COLOR_BLOOD_MAGIC))
	deactivate_power()
