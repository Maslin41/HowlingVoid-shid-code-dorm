/datum/antagonist/vampire
	...
	hijack_speed = 0.5
	stinger_sound = 'code/modules/antagonists/vampire/sound/lunge_warn.ogg'
	hud_icon = 'code/modules/antagonists/vampire/icons/antag_hud.dmi'
...
/datum/antagonist/vampire/remove_innate_effects(mob/living/mob_override)
	...
	if(rank_display)
		vampire_hud.infodisplay -= rank_display
...
/datum/antagonist/vampire/proc/on_hud_created(datum/source)
	...
	blood_display = new(null, vampire_hud)
	rank_display = new(null, vampire_hud)
	humanity_display = new(null, vampire_hud)
	vampire_hud.infodisplay += blood_display
	vampire_hud.infodisplay += rank_display
	vampire_hud.infodisplay += humanity_display
...
/datum/antagonist/vampire/proc/claim_coffin(obj/structure/closet/crate/claimed)
	...
	if(!coffin_turf)
		...
		if(is_type_in_typecache(current_area, banned_areas_typecache) || (istype(current_area, /area/ruin) && current_area.outdoors))
			valid_lair_area = FALSE
	if(!valid_lair_area)
...
/datum/action/antag_info/vampire
	name = "Vampire Guide"
	background_icon = 'code/modules/antagonists/vampire/icons/actions_vampire.dmi'
...
/datum/antagonist/vampire/proc/on_examine(datum/source, mob/examiner, list/examine_text)
	...
	var/text
	if(prince)
		text = "<img class='icon' src='\ref['code/modules/antagonists/vampire/icons/vampiric.dmi']?state=prince'> "
	else if(scourge)
		text = "<img class='icon' src='\ref['code/modules/antagonists/vampire/icons/vampiric.dmi']?state=scourge'> "
	else
		text = "<img class='icon' src='\ref['code/modules/antagonists/vampire/icons/vampiric.dmi']?state=vampire'> "
...
/datum/antagonist/vampire/proc/after_expose_reagents(atom/target, list/reagents, datum/reagents/source, methods)
	SIGNAL_HANDLER
