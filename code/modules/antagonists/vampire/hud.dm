...
/atom/movable/screen/vampire/rank_counter/Click()
	...
	msg += span_cult("Unarmed damage: +[owner_vamp.current_damage_bonus]")
...
/datum/antagonist/vampire/proc/update_hud()
	...
	if(rank_display)
		if(vampire_level_unspent > 0)
			rank_display.icon_state = "[initial(rank_display.icon_state)]_up"
		else
			rank_display.icon_state = initial(rank_display.icon_state)
		rank_display.maptext = FORMAT_VAMPIRE_HUD_TEXT("#ffd8d8", vampire_level)
