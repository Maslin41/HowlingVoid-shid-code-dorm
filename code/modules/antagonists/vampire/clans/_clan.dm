...
/datum/vampire_clan
	...
	var/join_icon = 'code/modules/antagonists/vampire/icons/clan_icons.dmi'
...
/datum/vampire_clan/New(datum/antagonist/vampire/owner_datum)
	...
	vampiredatum.owner.current.playsound_local(get_turf(vampiredatum.owner.current), 'code/modules/antagonists/vampire/sound/VampireAlert.ogg', 80, FALSE, pressure_affected = FALSE, use_reverb = FALSE)
...
/datum/vampire_clan/proc/spend_rank(mob/living/carbon/carbon_vampire)
	...
	for(var/datum/discipline/discipline as anything in vampiredatum.owned_disciplines)	// We do owned_disciplines, not clan_disciplines. clan_disciplines is used to populate owned_disciplines.
		if(discipline.get_abilities_with_level("next"))
			...
			option.image = image(icon = 'code/modules/antagonists/vampire/icons/disciplines.dmi', icon_state = discipline.icon_state)
...
/datum/vampire_clan/proc/on_vampire_broke_masquerade(datum/source, datum/antagonist/vampire/masquerade_breaker)
	...
	var/breaker_name = masquerade_breaker.owner.name || masquerade_breaker.owner.current.real_name || masquerade_breaker.owner.current.name
	to_chat(vampiredatum.owner.current, span_userdanger("[breaker_name] has broken the Masquerade! We must destroy them at all costs, for the good of all kindred!\n\
	...
	masquerade_objective.explanation_text = "Ensure [breaker_name], who has broken the Masquerade, succumbs to Final Death."
