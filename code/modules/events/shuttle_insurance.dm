...
/datum/round_event/shuttle_insurance/announce(fake)
	priority_announce("Incoming subspace communication. Secure channel opened at all communication consoles.", "Incoming Message", SSstation.announcer.get_rand_report_sound(), text_ru = "Получена входящая подпространственная передача. На всех консолях связи открыт защищённый канал.", title_ru = "Входящее сообщение")
...
/datum/round_event/shuttle_insurance/proc/answered()
	if(EMERGENCY_AT_LEAST_DOCKED)
		priority_announce("You are definitely too late to purchase insurance, my friends. Our agents don't work on site.", sender_override = ship_name, color_override = "red", text_ru = "Вы определённо опоздали с покупкой страховки, друзья мои. Наши агенты на месте не работают.")
		return
	if(insurance_message && insurance_message.answered == 1)
		var/datum/bank_account/station_balance = SSeconomy.get_dep_account(ACCOUNT_CAR)
		if(!station_balance?.adjust_money(-insurance_evaluation))
			priority_announce("You didn't send us enough money for shuttle insurance. This, in the space layman's terms, is considered scamming. We're keeping your money, scammers!", sender_override = ship_name, color_override = "red", text_ru = "Вы прислали недостаточно денег для страховки шаттла. На простом космическом языке это называется мошенничеством. Деньги мы оставляем себе, мошенники!")
			return
		priority_announce("Thank you for purchasing shuttle insurance!", sender_override = ship_name, color_override = "red", text_ru = "Благодарим за покупку страховки шаттла!")
