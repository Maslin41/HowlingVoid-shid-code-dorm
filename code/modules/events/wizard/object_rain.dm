...
/datum/round_event/wizard/object_rain/announce(fake)
	priority_announce("Anomalous weather patterns forecast in the [target_region].", "Janitorial alert", text_ru = "В районе [target_region] прогнозируются аномальные погодные условия.", title_ru = "Предупреждение уборочной службы")
