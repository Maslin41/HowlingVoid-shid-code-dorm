...
//Serves as a nice mechanic to people get ready for the launch.
	minor_announce(
		"Auxiliary base landing zone coordinates locked in for [A]. Launch command now available!",
		message_ru = "Координаты посадочной зоны вспомогательной базы зафиксированы для [A]. Команда запуска теперь доступна!",
	)
