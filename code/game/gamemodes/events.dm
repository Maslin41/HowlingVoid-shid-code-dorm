...
/proc/power_failure()
	priority_announce(
		...
		ANNOUNCER_POWEROFF,
		text_ru = "В электросети [station_name()] обнаружена аномальная активность. В качестве меры предосторожности питание станции будет отключено на неопределённый срок.",
		title_ru = "Критическая авария энергосети",
	)
...
/proc/power_restore()
	priority_announce(
		"Power has been restored to [station_name()]. We apologize for the inconvenience.",
		"Power Systems Nominal",
		ANNOUNCER_POWERON,
		text_ru = "Энергоснабжение [station_name()] восстановлено. Приносим извинения за доставленные неудобства.",
		title_ru = "Энергосистема в норме",
	)
	for(var/obj/machinery/power/apc/C as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/apc))
		if(C.cell && is_station_level(C.z))
...
/proc/power_restore_quick()
	priority_announce(
		"All SMESs on [station_name()] have been recharged. We apologize for the inconvenience.",
		"Power Systems Nominal",
		ANNOUNCER_POWERON,
		text_ru = "Все СМЭС на [station_name()] были перезаряжены. Приносим извинения за доставленные неудобства.",
		title_ru = "Энергосистема в норме",
	)
	var/list/all_smes = SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/smes)
	for(var/obj/machinery/power/smes/smes as anything in all_smes)
