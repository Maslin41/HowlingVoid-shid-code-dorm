...
- AI agents should:
  ...
  - Никогда не запускать сборку или компиляцию проекта автоматически; **Копилот не должен запускать `BUILD.bat`, `tools/build/*`, `bin/tgui-build.cmd` или другие сборочные скрипты** — запуск сборки выполняет человек.
  - **Перед проверкой ошибок (get_errors), а так же после починки ошибок** AI **обязан** сначала выполнить команду VS Code `dreammaker.reparse` (через `run_vscode_command`), чтобы линтер DreamMaker перечитал `.dme` и обновил диагностику. Без этого линтер может показывать устаревшие/кешированные ошибки из предыдущего состояния файлов.
  - **Если нужен ручной заменитель `reparse`:** использовать task `dm: refresh diagnostics`. Он запускает узкую DM-компиляцию через уже существующий `dm: build - tgstation.dme`, обновляет DM diagnostics и подходит для случаев, когда ошибки проявляются только после компиляции или `Problems` не синхронизируются после обычного `reparse`. Этот task сам по себе не запускает DreamSeeker или DreamDaemon; запуск игры/сервера остаётся только у launch-конфигураций.
  - **Практическое замечание по вызову `dreammaker.reparse`:** в некоторых сессиях `run_vscode_command` может ложно сообщать, что команда не найдена, если использовать обычную предварительную проверку. В таком случае AI должен вызывать `run_vscode_command` для `dreammaker.reparse` с `skipCheck: true`; сам вызов команды при этом может успешно отработать.
  - **Важно про «застрявшие» diagnostics:** `dreammaker.reparse` обновляет диагностику DreamMaker, но не всегда очищает старые записи из Problems, пришедшие от build task / `$dreammaker` problem matcher. Если `get_errors` продолжает ссылаться на код, которого уже нет в файле, AI должен считать такие diagnostics потенциально устаревшими и сверять их с текущим содержимым файла. Для надёжного обновления таких записей нужен новый прогон существующего build task человеком или перезагрузка окна VS Code.
  - **Важно про «ошибки только после компиляции»:** `dreammaker.reparse` следует считать быстрым sync-шагом, но не авторитетной заменой компиляции. Если ошибка проявляется только после build task, при расхождении между `reparse` и результатом компиляции приоритет всегда у результата компиляции. В качестве более узкой проверки DM-кода, если не нужен полный `Build All`, человек может запустить существующий task `dm: build - tgstation.dme`.
