...
/datum/augment_item/limb/apply(mob/living/carbon/human/augmented, character_setup = FALSE, datum/preferences/prefs)
	...
	if(digi_legs && ispath(path, /obj/item/bodypart/leg) && !(new_limb_path::bodyshape & BODYSHAPE_DIGITIGRADE))
		var/digi_type
		if(ispath(path, /obj/item/bodypart/leg/left))
			var/obj/item/bodypart/leg/left/new_leg = path
			digi_type = initial(new_leg.digitigrade_type)
		else if(ispath(path, /obj/item/bodypart/leg/right))
			var/obj/item/bodypart/leg/right/new_leg = path
			digi_type = initial(new_leg.digitigrade_type)
		if(digi_type)
