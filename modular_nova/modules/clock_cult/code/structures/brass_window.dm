...
/obj/structure/window/reinforced/clockwork/Initialize(mapload, direct)
	...
	if(on_reebe(src))
		...
		max_integrity = round(max_integrity * 0.5)
	return ..()

/obj/structure/window/reinforced/clockwork/attackby_secondary(obj/item/tool, mob/user, params)
	if(state == RWINDOW_SECURE && tool.tool_behaviour == TOOL_WIRECUTTER && IS_CLOCK(user))
...
/obj/structure/window/reinforced/clockwork/examine(mob/user)
	. = ..()
	if(IS_CLOCK(user) && state == RWINDOW_SECURE)
...
/obj/structure/window/reinforced/clockwork/proc/ratvar_act()
	return FALSE
