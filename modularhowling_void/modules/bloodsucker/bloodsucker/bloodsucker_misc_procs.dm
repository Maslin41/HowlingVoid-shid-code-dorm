...
/datum/antagonist/bloodsucker/proc/on_examine(datum/source, mob/examiner, examine_text)
	...
	if(info_to_display)
		var/img_html = "<img class='icon' src='\ref['modularhowling_void/modules/bloodsucker/icons/vampiric.dmi']?state=bloodsucker'></img>"
...
/datum/antagonist/bloodsucker/proc/break_masquerade(mob/admin, silent = FALSE)
	if(broke_masquerade)
		return
	if(!silent)
		owner.current.playsound_local(null, 'modularhowling_void/modules/bloodsucker/sounds/lunge_warn.ogg', 100, FALSE, pressure_affected = FALSE)
...
/atom/movable/proc/do_power_icon_animation(power_icon)
	var/mutable_appearance/alert = mutable_appearance('modularhowling_void/modules/bloodsucker/icons/actions_bloodsucker.dmi', power_icon)
