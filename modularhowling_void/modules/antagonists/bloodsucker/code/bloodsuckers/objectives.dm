...
/datum/objective/bloodsucker/vassalhim/update_explanation_text()
	...
	if(target?.current)
		explanation_text = "Убедитесь, что [vamp_victim.name], [!target_department_type ? target.assigned_role.title : SSjob.get_job(vamp_victim.job)] вассализованы с помощью стойки убеждения."
	else
